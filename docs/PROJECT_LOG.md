...
- Logged a Q&A clarifying how to compute nest-relative `x`/`y` displacement for `firmware/run_policy.py`: maintain local `(x, y, theta)` state, apply turn before translation to match `execute_action(...)`, and update position with `x += d * cos(theta)`, `y += d * sin(theta)`.
...
- Added targeted explanatory comments to [mission_control/main.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/main.py) covering CLI intent, shared world-state ownership, protocol dispatch behavior, per-port serial setup, headless PyGame initialization, the main UI loop, and shutdown sequencing.
- Updated [firmware/run_policy.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run_policy.py) to maintain a simple local pose estimate from issued turn/move commands, starting at the nest/origin by default, with an optional `--initial-heading-deg` flag and debug output for `x_mm`, `y_mm`, heading, and commanded distance.
- Updated [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) to support an optional BLE command-center client using a line-oriented `POS` / `SENSE` / `PHER` flow, inject returned `PHER_RESP` values into the observation vector, derive nest-relative pose from local action integration, and expose the current pheromone-awareness radius (`4.5 * agent_radius`, currently `31.5 cm` by default) in runtime settings.
- Extracted the BLE command-center transport from [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) into the new helper module [firmware/server_ble.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/server_ble.py), and updated `run.py` to import and use that dedicated transport file instead of embedding the BLE client class inline.
- Added targeted explanatory comments to [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) around CLI grouping, simulator-compatible observation assembly, pose integration semantics, command-center BLE handoff, cm/mm conversion, and the current placeholder pheromone-deposit trigger.
- Added an optional BLE peripheral backend to [mission_control/comms/receiver.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/comms/receiver.py), exposed matching CLI/config flags in [mission_control/main.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/main.py) and [mission_control/config.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/config.py), documented the new transport in [docs/MISSION_CONTROL.md](/Users/christopherlin/dev/cwsf2026/sim/docs/MISSION_CONTROL.md), and added the optional `bless` dependency needed for BLE GATT peripheral support.
...
- Updated the Pi/Mission Control pheromone path so robots can place new pheromones over Bluetooth in normal operation: [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) now enables `PHER` deposits by default with spacing based on the current awareness radius, and [mission_control/core/pheromone_field.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/core/pheromone_field.py) now applies each deposit as a circular field spread using the same `4.5 * agent_radius` radius rather than a single-cell write.
...
- Updated [firmware/ant.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/ant.py) to defer annotation evaluation and raise a clear runtime error when `serial.Serial` is unavailable, and logged the matching Q&A explaining that the usual fix is installing `pyserial` in the active environment and removing the unrelated `serial` package if present.
...
- Added more detailed explanatory comments to [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) and [firmware/bluetooth.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/bluetooth.py), covering the observation contract, scan bucketing, normalization choices, local pose semantics, control-loop stages, BLE line reconstruction, request/response matching, and failure-handling behavior.
...
- Extended the Mission Control BLE transport in [mission_control/comms/receiver.py](/Users/christopherlin/dev/cwsf2026/sim/mission_control/comms/receiver.py) to multiplex multiple robots over the shared BLE line channel by deriving logical per-robot labels from `robot_id`, and updated the Q&A to reflect that multi-robot BLE now works via protocol-level tagging rather than per-connection BLE workers.
...
- Expanded [MISSION_CONTROL.md](/Users/christopherlin/dev/cwsf2026/sim/docs/MISSION_CONTROL.md) with dedicated BLE documentation covering the GATT UUIDs, robot/Mission Control message flow, digital pheromone deposits, multi-robot BLE multiplexing by `robot_id`, and the expected firmware-side runtime behavior.
