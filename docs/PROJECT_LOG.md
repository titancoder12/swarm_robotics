...
- Logged a Q&A clarifying that there is no `agent0.py` entrypoint, that `checkpoints/agent_0.pt` is a model checkpoint, and that the current runtime entrypoints are `train/*.py` for simulation and `firmware/run_policy.py` for robot inference.
- Logged a Q&A clarifying how to use `checkpoints/agent_0.pt`: via `firmware/run_policy.py` for robot inference or `train/demo.py` for simulator playback, and noted that the runner expects a checkpoint directory rather than a direct `.pt` path.
...
- Logged a Q&A explaining that `command_center/comms` talks to each robot over its own line-oriented TCP or serial connection, with newline-delimited protocol messages carrying `robot_id` and replies returning on the same connection.
...
- Logged a Q&A clarifying that the deployed robot runtime in `firmware/run_policy.py` is policy-deterministic because it selects actions with `torch.argmax`, with any real-world variability coming from observations or hardware rather than action sampling.
...
- Audited and cleaned `docs/QandA.md` to remove stale historical entries, delete references to removed `pi/` and `robot/` paths, replace outdated `AntSwarmFirmware/` references with the current `firmware/` paths, and keep only repo-current questions that still matter.
- Restored `docs/QandA.md` to match revision `2d5293afcb7e02caca21a50fb2312411fc93bdb4` at the user’s request.
- Logged a Q&A explaining that `firmware/run_policy.py` is the live robot-side inference loop: parse args, load checkpoint, read scans, build observations, run greedy action selection, map actions to motion commands, and send them over serial at a fixed control rate.
- Logged a Q&A noting that `firmware/run_policy.py` was first added on 2026-03-21 in commit `e96d76ae8981201e10d5fe2a339548c8d900e425` with subject `Added saved models`.
...
- Added a new isolated `command_center/` subsystem with thread-safe world state, simulator-aligned digital pheromone field sampling, threaded TCP/serial receivers, a PyGame renderer, and a `fake_robot.py` harness for local testing without hardware.
...
- Added targeted explanatory comments to the non-obvious parts of the `command_center/` package, including protocol framing, per-connection worker behavior, centered pheromone-grid mapping, simulator-matched pheromone sampling, snapshot semantics, and world-to-screen coordinate conversion.
- Logged a Q&A clarifying that the current `command_center/` transport layer does not implement a Bluetooth-specific backend; it currently supports TCP sockets and optional serial ports only.
...
- Added [docs/prompts/11_bluetooth_prompt.md](/Users/christopherlin/dev/cwsf2026/sim/docs/prompts/11_bluetooth_prompt.md), a reusable implementation prompt for adding Bluetooth transport to the command center without changing the existing protocol, pheromone semantics, or `firmware/` isolation boundaries.
- Added [docs/BLUETOOTH_FIRMWARE_NOTES.md](/Users/christopherlin/dev/cwsf2026/sim/docs/BLUETOOTH_FIRMWARE_NOTES.md), documenting how the current `firmware/` runtime should add a separate command-center Bluetooth client alongside `ESP32Robot`, where it fits in `run_policy.py`, and what repo-specific gaps remain around pose estimation, pheromone injection, and deposit events.
...
- Clarified the Bluetooth docs to make the repo architecture explicit: in this codebase the command-center Bluetooth client belongs on the Raspberry Pi side in `firmware/`, not on the ESP32 motion/sensor controller, because `firmware/run_policy.py` owns observation assembly and model inference.
