...
- Logged a Q&A explaining that `server/comms` talks to each robot over its own line-oriented TCP or serial connection, with newline-delimited protocol messages carrying `robot_id` and replies returning on the same connection.
...
- Logged a Q&A recommending a shared pheromone-data structure: keep the mutable field authoritative in `server/`, expose a small shared contract for config/query/deposit/response data, and have `firmware/` consume sampled values rather than maintain its own competing field state.
...
- Added a new isolated `server/` subsystem with thread-safe world state, simulator-aligned digital pheromone field sampling, threaded TCP/serial receivers, a PyGame renderer, and a `fake_robot.py` harness for local testing without hardware.
...
- Added targeted explanatory comments to the non-obvious parts of the `server/` package, including protocol framing, per-connection worker behavior, centered pheromone-grid mapping, simulator-matched pheromone sampling, snapshot semantics, and world-to-screen coordinate conversion.
- Logged a Q&A clarifying that the current `server/` transport layer does not implement a Bluetooth-specific backend; it currently supports TCP sockets and optional serial ports only.
...
- Logged a Q&A clarifying that `server/` is the MacBook-side command-center package: it receives robot protocol messages, maintains the authoritative pheromone field and live world state, answers `SENSE` queries with `PHER_RESP`, and renders the PyGame operator UI.
- Added targeted explanatory comments to [server/main.py](/Users/christopherlin/dev/cwsf2026/sim/server/main.py) covering CLI intent, shared world-state ownership, protocol dispatch behavior, per-port serial setup, headless PyGame initialization, the main UI loop, and shutdown sequencing.
...
- Added an optional BLE peripheral backend to [server/comms/receiver.py](/Users/christopherlin/dev/cwsf2026/sim/server/comms/receiver.py), exposed matching CLI/config flags in [server/main.py](/Users/christopherlin/dev/cwsf2026/sim/server/main.py) and [server/config.py](/Users/christopherlin/dev/cwsf2026/sim/server/config.py), documented the new transport in [docs/COMMAND_CENTER.md](/Users/christopherlin/dev/cwsf2026/sim/docs/COMMAND_CENTER.md), and added the optional `bless` dependency needed for BLE GATT server support.
- Renamed stale `command_center` references to `server` across imports, README/doc run commands, internal links, and the Pi runtime variable naming so the new subsystem name is consistent with the actual `server/` package.
- Updated the Pi/server pheromone path so robots can place new pheromones over Bluetooth in normal operation: [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) now enables `PHER` deposits by default with spacing based on the current awareness radius, and [server/core/pheromone_field.py](/Users/christopherlin/dev/cwsf2026/sim/server/core/pheromone_field.py) now applies each deposit as a circular field spread using the same `4.5 * agent_radius` radius rather than a single-cell write.
- Logged a Q&A clarifying that robots can now deposit pheromones in the live BLE/server runtime, with deposits triggered by the current forward-motion spacing heuristic in `firmware/run.py` rather than by an explicit learned “deposit” action.
- Logged a Q&A describing how to test `firmware/run.py`, separating syntax/import validation, server protocol checks with `server/fake_robot.py`, and the real hardware run path that requires the ESP32 serial link and optional BLE server connection.
...
- Logged a Q&A clarifying how the server and robot connect across BLE/TCP/serial, and noting that multi-robot support is explicit for TCP and multiple serial ports while the current BLE backend should be treated as a more limited single-link transport unless extended further.
- Extended the server BLE transport in [server/comms/receiver.py](/Users/christopherlin/dev/cwsf2026/sim/server/comms/receiver.py) to multiplex multiple robots over the shared BLE line channel by deriving logical per-robot labels from `robot_id`, and updated the Q&A to reflect that multi-robot BLE now works via protocol-level tagging rather than per-connection BLE workers.
