...
- Logged a Q&A recommending a shared pheromone-data structure: keep the mutable field authoritative in `command_center/`, expose a small shared contract for config/query/deposit/response data, and have `firmware/` consume sampled values rather than maintain its own competing field state.
...
- Added targeted explanatory comments to [command_center/main.py](/Users/christopherlin/dev/cwsf2026/sim/command_center/main.py) covering CLI intent, shared world-state ownership, protocol dispatch behavior, per-port serial setup, headless PyGame initialization, the main UI loop, and shutdown sequencing.
...
- Extracted the BLE command-center transport from [firmware/run.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/run.py) into the new helper module [firmware/command_center_ble.py](/Users/christopherlin/dev/cwsf2026/sim/firmware/command_center_ble.py), and updated `run.py` to import and use that dedicated transport file instead of embedding the BLE client class inline.
...
- Added an optional BLE peripheral backend to [command_center/comms/receiver.py](/Users/christopherlin/dev/cwsf2026/sim/command_center/comms/receiver.py), exposed matching CLI/config flags in [command_center/main.py](/Users/christopherlin/dev/cwsf2026/sim/command_center/main.py) and [command_center/config.py](/Users/christopherlin/dev/cwsf2026/sim/command_center/config.py), documented the new transport in [docs/COMMAND_CENTER.md](/Users/christopherlin/dev/cwsf2026/sim/docs/COMMAND_CENTER.md), and added the optional `bless` dependency needed for BLE GATT server support.
