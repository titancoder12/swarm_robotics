...
def _rect_from_gap(x: int, width: int, gap_center: float, gap_height: float, arena_height: int) -> list[pygame.Rect]:
    gap_top = int(max(120, gap_center - gap_height * 0.5))
    gap_bottom = int(min(arena_height - 120, gap_center + gap_height * 0.5))
    return [
        pygame.Rect(x, 0, width, max(0, gap_top)),
        pygame.Rect(x, gap_bottom, width, max(0, arena_height - gap_bottom)),
    ]


def apply_seeded_chokepoint_world(env: SwarmEnv, seed: int) -> np.ndarray:
    width = int(env.width)
    ...
    gap_height = 150.0
    wall_width = 90
    wall1_x = int(width * 0.37)
    wall2_x = int(width * 0.63)
    gap1_center = float(height * 0.30 + rng.integers(-45, 46))
    gap2_center = float(height * 0.70 + rng.integers(-45, 46))

    env.obstacles = []
    env.obstacles.extend(_rect_from_gap(wall1_x, wall_width, gap1_center, gap_height, height))
    env.obstacles.extend(_rect_from_gap(wall2_x, wall_width, gap2_center, gap_height, height))

    nest_y = float(height * 0.50 + rng.integers(-30, 31))
    target_y = float(height * 0.50 + rng.integers(-75, 76))
    env.nest_position = (170.0, nest_y)
    env.targets = [(float(width - 170), target_y)]
...
def run_episode(env: SwarmEnv, policy: dict[str, Any], seed: int) -> dict[str, Any]:
    env.reset(seed=seed)
    obs = apply_seeded_chokepoint_world(env, seed)
    cfg = env.cfg
...
def write_notes() -> None:
    text = """# Chokepoint Route-Reuse Hypothesis Study: Stage max_10
...
def main() -> None:
    ...
    manifest = {
        ...
        "design": "fixed chokepoint route-reuse hypothesis study",
        "task_overrides": {
            "active_targets": 1,
            "target_respawn": False,
            "food_source_capacity": 18,
            "n_obstacles": 4,
            "max_steps": 1600,
            "layout": "two vertical wall barriers with offset gaps",
        },
        "extension_rule": {
            "highest_size": 10,
