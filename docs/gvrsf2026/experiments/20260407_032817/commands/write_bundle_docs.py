...
def main() -> None:
    ...
    report = f"""# Single-Gap Chokepoint Report: Stage max_10

## Bundle

**Bundle folder:** `docs/gvrsf2026/experiments/20260407_032817/`

## Abstract

This bundle tests the hypothesis that pheromone-based stigmergy becomes more valuable when the swarm must repeatedly traverse a single narrow passage but the task remains solvable enough to avoid floor effects. The study uses same-curriculum matched checkpoints, one repeated source, a fixed single-wall chokepoint layout, a longer horizon, and swarm sizes `1`, `3`, `6`, and `10` with `20` paired seeds per condition.
...
"""
    ...
    paper = f"""# Single-Gap Chokepoint Stigmergy Study
