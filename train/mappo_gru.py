...
def _evaluate(actor, critic, cfg, device, episodes: int, seed: int):
...
def train(args):
    ...
    for stage_index, stage in enumerate(curriculum, start=1):
        ...
        critic = CentralizedGRUCritic(spaces.state_dim, hidden_size=mappo_cfg.hidden_size).to(device)
        actor_opt = optim.Adam(actor.parameters(), lr=mappo_cfg.lr)
        critic_opt = optim.Adam(critic.parameters(), lr=mappo_cfg.lr)
        if resume_path:
            resume_info = _load_checkpoint(resume_path, actor, critic, actor_opt, critic_opt, device)
            carried_actor_state = actor.state_dict()
            resume_path = ""
        elif carried_actor_state is not None:
        ...
        while stage_steps < stage.total_steps:
            rollout = {
                ...
                "state": np.zeros((mappo_cfg.rollout_steps, spaces.state_dim), dtype=np.float32),
            ...
            for t in range(mappo_cfg.rollout_steps):
                ...
                next_state = env.state().astype(np.float32)
                rewards = np.array([rewards_dict[a] for a in agent_ids], dtype=np.float32)
                ...
                if done:
                    ...
                    obs_dict, _ = env.reset(seed=args.seed + completed_episodes + stage_index)
                    obs = np.stack([obs_dict[a] for a in agent_ids], axis=0).astype(np.float32)
                    state = env.state().astype(np.float32)
                    actor_hidden = actor.initial_hidden(cfg.n_agents, device)
                    critic_hidden = critic.initial_hidden(1, device)
            ...
            if args.eval_every > 0 and global_step > 0 and global_step % args.eval_every < cfg.n_agents:
                eval_metrics = _evaluate(actor, critic, cfg, device, args.eval_episodes, args.seed + 10_000 + stage_index)
                if eval_metrics:
                    eval_logger.log(
                        {
                            "stage": stage.name,
                    ...
                    print(
                        f"[MAPPO][Eval] {stage.name} step={global_step} "
